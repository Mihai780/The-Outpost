    ...
    void Start()
    {
        ...
        for(int x=1;x<=repetariPeAxaX;x++)
        {
            for(int y=1;y<=urcariPeAxaY;y++)
            {
                ...
                if (Physics2D.CircleCast(currentPoint, 0.1f, currentPoint) && Physics2D.CircleCast(currentPoint + addTile1, 0.1f, currentPoint + addTile1))
                {
                ...
                else if (!Physics2D.CircleCast(currentPoint + addTile1, 0.1f, currentPoint + addTile1) && !Physics2D.CircleCast(currentPoint, 0.1f, currentPoint))
                {
